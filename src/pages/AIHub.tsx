...
const BackupIcon = ({ status }: { status: Conversation['backupStatus'] }) => {
  switch (status) {
    case 'synced': return <CloudCheck className="h-3 w-3 text-green-500" />;
    case 'pending': return <Cloud className="h-3 w-3 text-yellow-500 animate-pulse" />;
    default: return <CloudOff className="h-3 w-3 text-gray-300" />;
  }
};
        ...
        <Card className="md:col-span-1 bg-[#121225]/50 border-white/5 backdrop-blur-xl">
          <CardHeader className="pb-3 border-b border-white/5 bg-white/5">
            ...
            <div className="relative mt-4">
              <Search className="absolute left-3 top-3 h-3.5 w-3.5 text-indigo-400/50" />
              <input 
                type="text" 
          ...
          <div className="p-6 bg-white/5 border-t border-white/5 space-y-4 relative z-10 backdrop-blur-xl">
            <div className="flex items-center justify-between px-2">
              <div className="flex items-center space-x-6">
                ...
                >
                  <Microscope className={`h-4 w-4 ${researchMode ? 'animate-pulse' : ''}`} />
            ...
            <div className="relative group">
              ...
              <div className="absolute right-3 top-3 flex items-center space-x-2">
                ...
                >
                  {isThinking ? <Loader2 className="h-5 w-5 animate-spin" /> : <Zap className="h-5 w-5" />}
                </button>
              </div>
            </div>
          </div>
